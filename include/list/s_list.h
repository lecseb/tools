 ...
 */
#ifndef _TOOLS_INCLUDE_LIST_S_LIST_H_
# define _TOOLS_INCLUDE_LIST_S_LIST_H_
...
/**
 * @brief The list struct is used for each element in a linked list
 ...
 */
# define s_list_next(list) ((list) ? (list)->next : NULL)

/**
 * @brief A convenience macro to get the data in an element.
 */
# define s_list_data(list) ((list) ? (list)->data : NULL)

/**
 * @brief Frees all of the memory used by a s_list. If list elements contain
 * dynamically-allocated memory, you should use s_list_free_full() or
 ...
 * @return the start of the new list that holds the same data as list
*/
export struct s_list *s_list_deep_copy(struct s_list *list,
	t_copy_func func);
 ...
 */
export struct s_list *s_list_concat(struct s_list *list1,
	struct s_list *list2);
...
#endif /* !_TOOLS_INCLUDE_LIST_S_LIST_H_ */
